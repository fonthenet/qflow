-- Terminal events (cancelled/served/no_show) must always notify the customer,
-- regardless of session state. Previously, has_session checked state='active'
-- only, which silently dropped notifications when the session had been closed
-- by an earlier event or cleanup.
...
BEGIN
  ...
  -- NO SHOW (terminal — fire even if session is closed)
  ...
  -- CANCELLED (terminal — fire even if session is closed)
