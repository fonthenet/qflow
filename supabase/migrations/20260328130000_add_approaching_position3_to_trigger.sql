-- Add "approaching" (position #3) reminder to the notify_ticket_called trigger.
-- When a ticket status changes, check if the ticket now at position 3 has an
-- active WhatsApp/Messenger session and hasn't been notified yet.
-- Also includes wait_minutes from org settings for called/recall messages.
