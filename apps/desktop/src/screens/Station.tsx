...
export function Station({ session, locale, isOnline, staffStatus, queuePaused, onStaffStatusChange, onQueuePausedChange, onSessionPatch }: Props) {
  ...
  const bookInHouse = async (data: { department_id: string; service_id?: string; customer_data: { name?: string; phone?: string; reason?: string; wilaya?: string }; priority: number; source: string; appointment_id?: string }) => {
    try {
      ...
      // Restaurant verticals (dine-in, takeout, delivery) all skip the
      // waiting → called → serving dance and go straight to 'serving' on
      // creation — there's no "customer to call to a desk" concept here:
      //   - dine-in: customer is at the table, kitchen preps now
      //   - takeout: customer is at the counter, kitchen preps now
      //   - delivery: rider arrives later, but the kitchen MUST start
      //     prepping immediately for the food to be ready in time
      // The legacy delivery 'waiting' branch made operators click Serve
      // to start kitchen prep, which delayed every order by an extra tap.
      ...
      const initialStatus = isRestaurantFlow ? 'serving' : 'waiting';
