...
function generateTicketNumber(db: Database.Database, officeId: string, deptCode: string, overrideDate?: string) {
  ...
  return `L-${deptCode}-${String(row.counter).padStart(4, '0')}`;
}
...
describe('Ticket number generation', () => {
  ...
  it('increments counter for same office/dept/date', () => {
    ...
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0001');
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0002');
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0003');
  });

  it('resets counter for different date', () => {
    expect(generateTicketNumber(db, 'o1', 'R', '2026-03-17')).toBe('L-R-0001');
    expect(generateTicketNumber(db, 'o1', 'R', '2026-03-17')).toBe('L-R-0002');
    // New day → resets
    expect(generateTicketNumber(db, 'o1', 'R', '2026-03-18')).toBe('L-R-0001');
  });

  it('separate counters per department code', () => {
    const today = '2026-03-18';
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0001');
    expect(generateTicketNumber(db, 'o1', 'S', today)).toBe('L-S-0001');
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0002');
    expect(generateTicketNumber(db, 'o1', 'S', today)).toBe('L-S-0002');
  });
  ...
  it('separate counters per office', () => {
    ...
    const today = '2026-03-18';
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0001');
    expect(generateTicketNumber(db, 'o2', 'R', today)).toBe('L-R-0001');
  });
  ...
  it('pads numbers to 4 digits', () => {
    ...
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-0100');
  });

  it('handles high volume (999+) without format break', () => {
    const today = '2026-03-18';
    db.prepare("INSERT INTO ticket_counter (office_id, dept_code, counter, date) VALUES ('o1', 'R', 9998, ?)").run(today);
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-9999');
    expect(generateTicketNumber(db, 'o1', 'R', today)).toBe('L-R-10000');
  });
});
