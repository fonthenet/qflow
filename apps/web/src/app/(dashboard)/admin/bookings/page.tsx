...
export default async function AdminBookingsPage() {
  ...
  const { data: appointments } = officeIds.length
    ? await supabase
        .from('appointments')
        .select(
          'id, office_id, department_id, service_id, staff_id, customer_name, customer_phone, customer_email, scheduled_at, created_at, status, ticket_id, calendar_token, recurrence_parent_id'
        )
        .in('office_id', officeIds)
