...
describe('POST /api/moderate-appointment', () => {
  ...
  it('complete: returns 200 and marks appointment completed', async () => {
    ...
    selectChain.in = vi.fn().mockResolvedValue({ data: [], error: null });
    selectChain.single = vi.fn().mockResolvedValue({ data: { ticket_id: null }, error: null });
    selectChain.then = (fn: any) => Promise.resolve({ data: [], error: null }).then(fn);
