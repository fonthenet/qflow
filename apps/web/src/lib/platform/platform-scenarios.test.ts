...
describe('multi-industry platform scenarios', () => {
  it('ships starter coverage for all configured verticals', () => {
    expect(industryTemplates).toHaveLength(12);
    expect(industryTemplates.map((template) => template.vertical)).toEqual(
      expect.arrayContaining([
        'public_service', 'bank', 'clinic', 'restaurant', 'barbershop',
        'education', 'telecom', 'insurance', 'automotive', 'legal', 'real_estate', 'other',
      ])
    );
  });
