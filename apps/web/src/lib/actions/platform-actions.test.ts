...
describe('platform actions', () => {
  ...
  it('applies an industry template and seeds starter office data end to end', async () => {
    ...
    expect(context.supabase.tables.desk_services.length).toBeGreaterThanOrEqual(3);
    ...
    expect(context.supabase.tables.offices[0]?.settings.platform_service_areas).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ id: 'dining-room' }),
        expect.objectContaining({ id: 'patio' }),
      ])
    );
