// Algerian wilayas (provinces) — official 58-wilaya list
export interface Wilaya {
  code: number;
  name: string;      // French / Latin
...
/**
 * Resolve a user-supplied wilaya input to a canonical Wilaya.
 * Accepts:
 *  - numeric code ("16", "16 - Alger")
 *  - French/Latin name (case- and accent-insensitive)
 *  - Arabic name
 */
export function resolveWilaya(input: string): Wilaya | null {
  ...
  const n = norm(raw);
  return (
    WILAYAS.find(w => norm(w.name) === n) ||
    WILAYAS.find(w => w.name_ar.trim() === raw.trim()) ||
    null
  );
}

/** Format as "16 - Alger" */
export function formatWilaya(w: Wilaya, locale: 'fr' | 'ar' | 'en' = 'fr'): string {
